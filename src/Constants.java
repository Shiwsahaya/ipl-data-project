public final class Constants {
    ...
    public static final String TOP_BOWLER="select bowler, cast(sum(cast(total_runs as float))/((count(cast(ball as float)))/6.0)as decimal(10,2)) as economy from deliveries where id in(select m_id from matches where season=?) group by bowler order by economy limit ?";
